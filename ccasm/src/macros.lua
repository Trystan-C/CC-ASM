...
origin = {
    ...
    verifyEach = function(operand)
        assert(operand.definition == operandTypes.immediateData, "origin: Operand must be immediate data.");
...
declareByte = {
    numOperands = 1,
    verifyEach = function(operand)
        assert(operand.definition == operandTypes.immediateData, "declareByte: Operand must be immediate data.");
...
declareWord = {
    numOperands = 1,
    verifyEach = function(operand)
        assert(operand.definition == operandTypes.immediateData, "declareWord: Operand must be immediate data.");
...
declareLong = {
    numOperands = 1,
    verifyEach = function(operand)
        assert(operand.definition == operandTypes.immediateData, "declareLong: Operand must be immediate data.");
...
declareString = {
    numOperands = 1,
    verifyEach = function(operand)
        assert(operand.definition == operandTypes.stringConstant, "declareString: Operand must be a string constant.");
    end,
    assemble = function(objectCode, operands)
        return operands[1].valueBytes;
    end
